...
public class BoardServiceImp implements BoardService{
	...
	public void insertBoard(BoardVO board, MemberVo user, MultipartFile[] files) {
		...
		if(board.getBd_title().trim().length() == 0)
			return;
		//내용에 공백만 있는 경우 (trim 글자와 글자사이 큰 공백은 인식안함) 
		...
		//System.out.println(board); //기본키 가져오는 지 확인 
		//첨부파일 작업
	...
	public void updateBoard(BoardVO board, MemberVo user) {
		...
		BoardVO bdBoard = boardDao.selectBoard(board.getBd_num());
		
		//작성자와 로그인한 회원이 다르면
		if(!user.getMe_id().equals(bdBoard.getBd_me_id()))
			return;
	...
	public String updateLikes(LikesVO likes) {
		...
		try {
			//해당 게시물에 추천/비추천이 처음이면 insert(데이터를 새로 추가해야함)
			if(dbLikes == null) {
