...
public class UploadFileUtils {
	 ...
	 * @return 서버위치와 업로드된 경로와 업로드된 파일명이 합쳐진 문자열
	 * */
	public static String uploadFile(String uploadPath, String originalName, byte[] 	
			fileData)throws Exception{
		//파일명에 붙일 UUID를 생성
		UUID uid = UUID.randomUUID();
		//savedName-> 업로드되서 저장될 파일명 
		String savedName = uid.toString() +"_" + originalName;
		//savedPath-> 서버위치에 업로드된 경로 : /년도/월/일
	...
	private static String calcPath(String uploadPath) {
		//현재 시간 정보를 가져옴 Calendar-> 년도.월.일을 따로따로 메소드로 가져옴 
	...
	/**
	 * uploadPath에 paths들이 있는 지 없는 지 확인해서 없으면 해당 폴더를 만듬 
	...
	private static void makeDir(String uploadPath, String... paths) {
		...
		for(String path : paths) {
			...
			//폴더가 없으면 폴더를 생성 
			if( !dirPath.exists())
				dirPath.mkdir(); //mkdir-> 폴더를 만드는 메소드 
		}
	}
