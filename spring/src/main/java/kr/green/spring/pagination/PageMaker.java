...
public class PageMaker {
	...
	private boolean prev;   //이전 버튼 활성화 여부(동자 방식에 따라 약간 다를 수 있음)
	...
	public void calcData() {
		...
		//endPage = (int) (Math.ceil(6(현재페이지네이션번호)/10.0(현재페이지의 페이지네이션번호갯수))*displayPageNum); 10
		//endPage = (int) (Math.ceil(criteria.getPage()/(double) displayPageNum)*displayPageNum);
		...
		//prev = criteria.getPage() == 1 ? false : true; //현재페이지가 첫번째 페이지이면 이전버튼 비활성화 
		/* 현재 페이지메이커에 마지막 페이지에 컨텐츠의 마지막이 포함되어 있으면 next가 없어야 함 */
		next = endPage * criteria.getPerPageNum() >= totalCount ? false:true; //마지막페이지네이션이면 다음버튼 비활성화
		//next = criteria.getPage() == endPage ? false : true; //현재페이지가 마지막페이지이면 다음버튼 비활성화 
	}
