...
public class Ex7_Mart {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2: 
				...
				//제품을 선택하고 입고할 수량을 선택해서 판매제품리스트에 제품이 입고 되면 
			...
			case 3:
				...
				printProductList(list, listCount);
				//제품을 선택하고 수량을 선택함 
				Product selecProduct = selectProduct(scan, list, listCount);
				//선택된 제품이 있으면
				if(selecProduct != null) {
					//장바구니에 선택된 제품을 담고 
					basket[basketCount]= selecProduct;
					//장바구니에 담긴 개수를 하나 증가
					basketCount++;
					//장바구니에 있는 목록을 출력 
			...
			case 4:
				...
				if(sum > buyPrice) {
					//결제를 취소할건지 물어봄 
					System.out.println("결제를 취소하겟습니까?(취소시 장바구니는 비워집니다. y/n): ");
					char cancel = scan.next().charAt(0);
					//--취소하면 
					if(cancel == 'Y' || cancel == 'y') {
					 //장바니구니에 담긴 제품들을 마트에 돌려줘야함
	...
	/* 기능 : 스캐너를 이용하여 제품과 수량을 선택하면 제품리스트에서 해당 제품을 수량에 맞게 
	 *       꺼내서 제품을 돌려누는 메소드 
	...
	public static Product selectProduct(Scanner scan, Product list[], int listCount) {
		...
		//선택된 제품을 복사해서 가져옴 //클래스타입변환-강제타입변환 
		if(buyProduct instanceof SnackBox) {
		 ...
		 //제고량보다 많은 수량을 입력한 경우 /제품 제고량에서 수량만큼 뺌
	...
	/* 기능 : 장바구니에 담은 제품들을 마켓에 돌려주는 메소드  //수량을 맞추는 것 
	...
	//판매제품 목록과 장바구니 제품목록을 하나씩 비교하여 제자리에 가져다 놓음 
	 for(int i=0 ; i<listCount ; i++) {
		 for(int j=0 ; j<basketCount ; j++ ) {
			 Product pi = list[i]; //판매제품
			 Product pj = basket[j]; //장바구니 제품
			 //판매제품 이름과 장바구니 제품이름이 같으면 
			 if(pi.getName().equals(pj.getName())) {
				 //pi.amount : 제고량, pj.amount : 구매햐려고 선택했던 수량 
				 //장바구니에 있는 제품을 제자리에 돌려놓기위해서 판매제품 수량에 장바구니 제품을 수량을 추가 
				 pi.sumAmount(pi.getAmount());;
			 	}
			 	}
	 		}
		 }
